...
var LoginService = /** @class */ (function () {
    ...
    LoginService.prototype.login_crm = function (userdata) {
...
var LoginComponent = /** @class */ (function () {
    function LoginComponent(appSettings, snackBar, loginservice, fb, dialog, router) {
    ...
    LoginComponent.prototype.onSubmit = function (values) {
        ...
        if (this.form.valid) {
            ...
            this.loginservice.login_crm(userdata).subscribe(function (res) {
                _this.users = res;
                console.log(_this.users);
                if (_this.users.token) {
                    localStorage.setItem('Token', _this.users.token);
                    if (_this.users.status == 200 && _this.users.error == false) {
                        var message = "Login Successfull.!";
                        var action = "OK";
                        _this.snackBar.open(message, action, {
                            duration: 2000,
                        });
                        _this.Wating_button = false;
